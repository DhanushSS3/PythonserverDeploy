...
# Import the margin calculator service and its helper
from app.services.margin_calculator import calculate_single_order_margin # No longer need calculate_base_margin_per_lot here for margin_to_add
...
async def process_new_order(
    ...
    # Use the calculate_single_order_margin from the margin_calculator service
    # This returns the margin that will be stored with the individual order.
    # This value is equivalent to the 'request_margin' in your Django snippet.
    full_calculated_margin_usd, adjusted_order_price, contract_value = await calculate_single_order_margin(
        db=db,
        redis_client=redis_client,
        user_id=user.id,
        order_quantity=new_order_quantity,
        order_price=order_request.order_price, # Use the original order price
        symbol=order_symbol,
        order_type=new_order_type
    )
    ...
    # --- Step 2: Fetch user's existing open positions for the same symbol (from DB) ---
    existing_positions = await crud_order.get_open_orders_by_user_id_and_symbol(
        db=db,
        user_id=user.id,
        symbol=order_symbol
    ...
    # --- Step 3 & 4: Calculate net existing position and unhedged/hedged quantities for the NEW order ---
    # This logic matches the Django snippet's approach to determine the unhedged part of the *new* order.
    net_existing_buy = max(Decimal(0), total_existing_buy_quantity - total_existing_sell_quantity)
    net_existing_sell = max(Decimal(0), total_existing_sell_quantity - total_existing_buy_quantity)
    ...
    # Calculate the unhedged quantity of the *new* order based on available existing opposition
    hedged_quantity = min(new_order_quantity, net_opposing_quantity)
    unhedged_quantity = new_order_quantity - hedged_quantity
    ...
    # --- Step 5: Calculate the margin amount to ADD to the user's overall margin based on the unhedged part ---
    # Implementing the logic derived from the Django snippet for 'margin_to_add'.
    # It uses the *full calculated margin of the new order* divided by the *new order quantity*
    # to get the margin per lot *of the new order*.
    margin_to_add = Decimal(0)

    if new_order_quantity > Decimal(0): # Prevent division by zero
         # Calculate margin per lot of the NEW order
         try:
             # Ensure full_calculated_margin_usd is Decimal (it should be from calculate_single_order_margin)
             margin_per_lot_new_order = full_calculated_margin_usd / new_order_quantity
             # Optional: Quantize margin_per_lot if needed for intermediate step, but Decimal handles precision
             # margin_per_lot_new_order = margin_per_lot_new_order.quantize(Decimal('0.00000001'), rounding=ROUND_HALF_UP) # Example quantization
         except InvalidOperation as e:
             logger.error(f"Invalid operation calculating margin per lot for new order ({full_calculated_margin_usd} / {new_order_quantity}): {e}", exc_info=True)
             raise OrderProcessingError("Failed to calculate margin per lot for hedging update.")

         # Calculate the margin to add based on the unhedged portion of the NEW order
         margin_to_add = margin_per_lot_new_order * unhedged_quantity

         logger.debug(f"Margin per lot of the NEW order: {margin_per_lot_new_order}")
         logger.debug(f"Margin to add to user overall margin (based on unhedged part of new order): {margin_to_add}")
    else:
         logger.debug("New order quantity is zero, margin to add is 0.")


    # --- Step 6: Fetch user with lock and perform margin check and update ---
    # Uses DB lock to get the latest overall margin and balance.
    db_user_locked = await crud_user.get_user_by_id_with_lock(db, user_id=user.id)
    ...
    logger.debug(f"User {user.id}: Wallet Balance (from DB): {user_wallet_balance}, Overall Margin BEFORE update (from DB): {user_overall_margin_before}, Available Free Margin: {available_free_margin}")
    logger.debug(f"Margin amount to ADD to user's overall margin: {margin_to_add}")


    # Check if user has sufficient free margin for the margin amount to add
    if available_free_margin < margin_to_add:
        logger.warning(f"Insufficient funds for user {user.id} to place order. Margin to add: {margin_to_add}, Available free margin: {available_free_margin}")
        # Quantize the margin_to_add for the error message for cleaner display
        margin_to_add_display = margin_to_add.quantize(Decimal('0.00000001'), rounding=ROUND_HALF_UP)
        available_free_margin_display = available_free_margin.quantize(Decimal('0.00000001'), rounding=ROUND_HALF_UP)
        raise InsufficientFundsError(f"Insufficient funds. Required additional margin: {margin_to_add_display:.8f} USD. Available free margin: {available_free_margin_display:.8f} USD.")


    # Update the user's total overall margin by ADDING the calculated margin_to_add
    db_user_locked.margin += margin_to_add
    # Optional: Quantize the user's total margin before saving if desired, though Decimal handles precision
    # db_user_locked.margin = db_user_locked.margin.quantize(Decimal('0.00000001'), rounding=ROUND_HALF_UP)

    logger.info(f"User {user.id}: Updated overall margin to {db_user_locked.margin} by adding {margin_to_add} (based on unhedged part of new order).")


    # --- Step 7: Prepare data for creating the UserOrder record and save ---
    # The margin stored in the order table is the FULL, non-hedged margin (full_calculated_margin_usd).
    # ... (existing order data preparation and DB save/commit) ...
    # unique_order_id = str(uuid.uuid4())
    order_status = "OPEN" # Or "PENDING_EXECUTION" depending on your flow
    ...
    try:
        ...
        # TODO: Post-order creation actions: Publish event, Update user portfolio cache for broadcaster.
        # The cache update should reflect the *new total* overall margin and the new position.
        # This update should also trigger a WebSocket message to the user via the broadcaster.
        # This will require recalculating the user's full portfolio PnL, Equity, Free Margin etc.
        # based on ALL open positions and CURRENT market prices.

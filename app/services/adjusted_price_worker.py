...
from app.core.cache import set_adjusted_market_price_cache, get_group_symbol_settings_cache, REDIS_MARKET_DATA_CHANNEL
    ...
    while True:
        try:
            ...
            try:
                ...
                continue
            # Remove non-symbol keys
            raw_market_data = {k: v for k, v in message_data.items() if k not in ["type", "_timestamp"]}
            ...
            logger.error(f"Error in adjusted_price_worker: {e}", exc_info=True)
        await asyncio.sleep(0.01) 